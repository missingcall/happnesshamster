...
class SendDynamicViewModel : BaseViewModel() {
    ...
    //播放时常
    val playDuration = MutableStateFlow<Int>(0)
    ...
    var isPictureSelected =MutableStateFlow(selectedMode.value == SelectMode.Picture)
    ...
    val isCouldSend =
        combine(
            ...
            selectImageCount,
            sendDynamicText
        ) { va,value0, value1, value2, value3 ->
            //只要其中一个条件满足就可以
            !va&&(!value0 && value1 || value0 && value2 > 0 || value3.isNotEmpty())
        }.asLiveData()
