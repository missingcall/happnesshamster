...
import com.kissspace.common.model.wallet.TransferRecord
    ...
    private fun getTransferRecord() {
        if (walletType == Constants.WalletType.COIN.type) {
            ...
            scopeNetLife {
                ...
                if (data.isNotEmpty()) {
                    mBinding.tvTransferNearlyTitle.visibility = View.VISIBLE
                    mBinding.rvTransferNearly.visibility = View.VISIBLE
                    mBinding.rvTransferNearly.linear().setup {
                        addType<TransferRecord> { R.layout.rv_trans_item }
                        onClick(R.id.layout_root) {
                            val model = getModel<TransferRecord>(modelPosition)
                            mBinding.etUserId.setText(model.targetUserId)
                        }
                    }.models = data
                }
            }
