...
class FamilyDetailActivity : BaseActivity(R.layout.mine_activity_family_detail) {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        mBinding.ivQuestion.safeClick {
            mViewModel.getQQNumber {
                CommonHintDialog.newInstance("如有问题请联系专属微信", it.orEmpty())
    ...
    private fun updateFamily(it: FamilyDetailInfoModel?) {
        ...
        if (it?.userFamilyStatus == Constants.FAMILY_NOT_APPLY) {
            mViewModel.familyNotice.set("公会还未发出公告~")
            mViewModel.familyNoticeColor.set(resources.getColor(com.kissspace.module_common.R.color.color_ui_sub_text_middle))
        } else {
            mViewModel.familyNotice.set(it?.familyNotice ?: "公会还未发出公告~")
            if (mViewModel.familyNotice.get().isNotEmptyBlank()) {
                mViewModel.familyNoticeColor.set(resources.getColor(com.kissspace.module_common.R.color.color_ui_main_text))
            } else {
                mViewModel.familyNoticeColor.set(resources.getColor(com.kissspace.module_common.R.color.color_ui_sub_text_middle))
            }
        ...
        if (it?.familyDesc.isNotEmptyBlank()) {
            ...
            if (mViewModel.isFamilyHeader.get() == true) {
                mViewModel.familyHint.set("公会长编辑的对本公会的介绍~")
            } else {
                mViewModel.familyHint.set("该公会没有任何介绍")
            }
        }
        ...
        if (it?.isFamilyHeader == true) {
            ...
            mBinding.layoutUnionTurnoverToday.safeClick {
                jump(RouterPath.PATH_FAMILY_PROFIT, "familyId" to familyId.orEmpty())
            }

            mBinding.layoutUnionTurnoverTotal.safeClick {
                jump(RouterPath.PATH_FAMILY_PROFIT, "familyId" to familyId.orEmpty(),"isToday" to false)
            }
            ...
            mBinding.tvAddFamily.safeClick {
                CommonConfirmDialog(this, "确定要申请解散公会吗", negativeString = "取消") {
                    if (this) {
                ...
                Constants.FAMILY_NOT_APPLY -> {
                    ...
                    mBinding.tvAddFamily.safeClick {
                        CommonConfirmDialog(
                            ...
                            "是否确认加入公会${mViewModel.familyDetailModel.get()?.familyName}",
                            ""
                        ) {
                            if (this) {
    ...
    override fun createDataObserver() {
        ...
        collectData(mViewModel.getFamilyMemberListModelEvent, onSuccess = {
            mBinding.rvFamily.bindingAdapter.models = it.familyMemberRecords
//            mBinding.rvFamily.bindingAdapter.addModels()
        })
