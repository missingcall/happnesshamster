...
object RoomServiceManager {
     ...
     * 第一次进房时间
     */
    var quitRoomTime = System.currentTimeMillis()
...
//    val roomList = mutableListOf<RoomListBean>()
     ...
     */
    fun init(context: Context,roomInfoBean: RoomInfoBean) {
        this.roomInfo?.crId?.let {
            if(it != roomInfoBean.crId){
                release{
                   initRoomInfo(context,roomInfoBean)
                }
            }
        }?: kotlin.run {
            initRoomInfo(context,roomInfoBean)
    ...
    private fun initRoomInfo(context: Context, roomInfoBean: RoomInfoBean) {
        this.roomInfo = roomInfoBean
        if (mZegoEngine == null) {
            firstJoinRoomTime = System.currentTimeMillis()
            quitRoomTime = System.currentTimeMillis()
            initZegoEngine()
            ...
            checkNimStatus()
            val intent = Intent(context, RoomNotificationService::class.java)
