...
class LiveViewModel : BaseViewModel(), DefaultLifecycleObserver {
    ...
    val roomInfoBean  by lazy { MutableLiveData<RoomInfoBean>() }

    val crId  by lazy { MutableLiveData<String>() }
...
//            })
    ...
    private val _gameListEvent = MutableSharedFlow<ResultState<List<RoomGameConfig>>>()
    val gameListEvent = _gameListEvent.asSharedFlow()
...
//    }
    ...
    fun getGameList() {
//        val param = mutableMapOf<String, Any?>()
//           request(AppConfigKey.ROOM_ACTIVE_PATH_CONFIG, Method.POST, param, state = _gameListEvent
//        )
    }

    fun getCurrentMessage(){
        request(RoomApi.API_HOME_QUERY_MESSAGE, Method.GET, state = _homeMessage)
    }
    ...
    fun handleRoomInfo(crId: String?, roomType: String?, stochastic: String?) {
