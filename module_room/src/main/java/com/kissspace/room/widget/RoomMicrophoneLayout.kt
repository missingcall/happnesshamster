...
class RoomMicrophoneLayout : ConstraintLayout {
    var mBinding: RoomLayoutMicrophoneBinding =
...
//        recyclerView.layoutManager?.findViewByPosition(position)?.findViewById<ImageView>(R.id.iv_user_talking)?.visibility = VISIBLE
    }

    fun stopTalking(model: MicUserModel) {
//        val recyclerView = getRecyclerView()
//        val position = recyclerView.getMutable<MicUserModel>()
//            .indexOfFirst { it.wheatPositionId == model.wheatPositionId }
//        recyclerView.layoutManager?.findViewByPosition(position)?.findViewById<ImageView>(R.id.iv_user_talking)?.visibility = INVISIBLE
    }
