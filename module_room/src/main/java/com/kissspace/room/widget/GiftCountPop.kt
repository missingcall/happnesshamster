...
//        data.add(CountItem(3344, "生生世世"))
        data.add(CountItem(-1, "生生世世"))
        data.reverse()
        mBinding.recyclerView.linear().setup {
            addType<CountItem> {
                val item = getModel<CountItem>(it)
                if(item.count == -1){
                    R.layout.room_layout_gift_count_item_define
                }else
                    R.layout.room_layout_gift_count_item
            }
            onBind {
                val line = findView<View>(R.id.line)
                line.visibility = if (modelPosition == mutable.size - 1) View.GONE else View.VISIBLE
                if(itemViewType == R.layout.room_layout_gift_count_item_define){
                    findView<View>(R.id.tv_commit).setOnClickListener {
                        val amount = findView<EditText>(R.id.tv_count).text.toString()
