...
class GiftDialog : BaseDialogFragment<RoomDialogGiftBinding>(RoomDialogGiftBinding::inflate,Gravity.BOTTOM) {
    ...
    private fun initEvent() {
        ...
        mBinding.viewPager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                showCheckAllGiftButton(position)
                mBinding.cbGiftLock.isChecked = false
                checkedModeChanged(false)
            }
        })

        //选择数量
        mBinding.layoutGiftSendCount.setOnClickListener {
            if(mBinding.cbGiftLock.visibility == View.VISIBLE&& mBinding.cbGiftLock.isChecked){
                customToast("请先完成礼物锁定")
        ...
        mBinding.cbGiftLock.setOnClickListener {
            if(mViewModel.isCheckedAllGift.get()!!){
                checkedAllGift(false)
                mViewModel.isCheckedAllGift.set(false)
                mBinding.tvCheckedAllUser.visibility = View.VISIBLE
                mBinding.recyclerUsers.bindingAdapter.singleMode = false
            }

            mBinding.cbGiftLock.isChecked.let {
                if(!it) {
                    //ture 表示完成锁定
                    mBinding.cbGiftLock.isChecked = true
                    checkedModeChanged(true)
                }else{
                    mViewModel.lockGift(getAllGift())
                }
            }
        }
        ...
        mBinding.tvSendGift.safeClick {
            ...
            val checkedUser =
                if (crId.isNotEmpty()) mBinding.recyclerUsers.bindingAdapter.getCheckedModels() else mutableListOf(
                    MicUserModel(wheatPositionId = userId)
                )
            if (checkedGift.isEmpty()) {
    ...
    override fun observerData() {
        ...
        collectData(mViewModel.sendGiftEvent, onSuccess = {
            customToast("送礼成功")
//            handleGiftMessage(it){
//                    gift, nickName, userIcon ->
//                BosonFriendDialog.newInstance(gift,nickName,userIcon).show(parentFragmentManager)
//            }
            if(mViewModel.isCheckedAllGift.get()!!){
                checkedAllGift(false)
                mViewModel.isCheckedAllGift.set(false)
            ...
            if (mBinding.viewPager.currentItem == mBinding.tabLayout.childCount - 1) {
                ...
                val currentPageGiftSize = (fragment as GiftListFragment).updateGiftList(it)
                if (mBinding.viewPager.currentItem == mBinding.tabLayout.childCount - 1) {
                    mBinding.tvCheckedAllGift.visibility = if (currentPageGiftSize ==0) View.GONE else View.VISIBLE
                    mBinding.cbGiftLock.visibility = if (currentPageGiftSize ==0) View.GONE else View.VISIBLE
                }
            }
        ...
        collectData(mViewModel.lockGiftEvent, onSuccess = {
            customToast("操作成功")
            if (mBinding.viewPager.currentItem == mBinding.tabLayout.childCount - 1) {
                val fragment =
                    childFragmentManager.findFragmentByTag("f${mBinding.viewPager.currentItem}")
                (fragment as GiftListFragment).updateLockGiftList(it)
            }
            mBinding.cbGiftLock.isChecked = false
            checkedModeChanged(false)
        }, onEmpty = {
    ...
    private fun getCheckedGift(): List<GiftModel> {
        ...
        return if(fragment is GiftListFragment) {
            fragment.getCheckedGift()
        } else mutableListOf()
    }


    private fun getAllGift(): List<GiftModel> {
        val fragment = childFragmentManager.findFragmentByTag("f${mBinding.viewPager.currentItem}")
        return if(fragment is GiftListFragment) {
            fragment.getAllGift() as  List<GiftModel>
        } else mutableListOf()
    }
    ...
    private fun checkedModeChanged(isLock: Boolean) {
        val fragment = childFragmentManager.findFragmentByTag("f${mBinding.viewPager.currentItem}")
        if(fragment!=null && fragment is GiftListFragment){
            fragment.checkModeChanged(isLock)
        }
    }
    ...
    private fun showCheckAllGiftButton(position: Int) {
        mBinding.rltDiamond.visibility = View.GONE
        if (position < mBinding.tabLayout.childCount - 1) {
            mBinding.tvCheckedAllGift.visibility = View.GONE
            mBinding.cbGiftLock.visibility = View.GONE
            mBinding.tabLayout.getChildAt(position).let {
                if(it is TextView){
                    logE("text"+it.text.toString())
                    if(it.text == "人气"){
                        mBinding.rltDiamond.visibility = View.VISIBLE
                    }
                }
            }
        } else {
            request<MutableList<PackGiftModel>>(RoomApi.API_GET_PACK_GIFT_LIST,
                Method.GET,
                onSuccess = {
                    (if (it.isEmpty()) View.GONE else View.VISIBLE).let { gone ->
                        mBinding.cbGiftLock.visibility = gone
                        mBinding.tvCheckedAllGift.visibility = gone
                    }
                },
                onError = {
                    mBinding.tvCheckedAllGift.visibility = View.GONE
