...
import com.kissspace.room.widget.CharmValueCleanDialog
...
import com.kissspace.util.logETag
...
abstract class BaseLiveFragment(layoutId: Int) : BaseFragment(layoutId),
    ...
    //聊天消息类型-派单
    private val CHAT_MSG_TYPE_PD = 4
...
//    protected val mMicUserList = mutableListOf<MicUserModel>()
        ...
        if (it.msgType == MsgTypeEnum.custom) {
            ...
            if (attachment.type == Constants.IMMessageType.MSG_TYPE_ROOM_CHAT_CONTENT && it.status == MsgStatusEnum.success) {
                val model = RoomChatMessageModel(
                    ...
                    aitList = if (attachment.data is RoomChatMessageModel) (attachment.data as RoomChatMessageModel).aitList else null
                )
                sendChatTask()
    ...
    val onMicrophoneClick: (MicUserModel) -> Unit = {
        when {
            if (it.wheatPositionId.isEmpty()) {
                true
            } else {
                false
            } -> {
                ...
                if (isNormalUser()) {
                    ...
                    else if (it.onMicroPhoneNumber == 0) {
                        customToast("该麦位只有管理和厅主可以上麦", true)
    ...
    override fun initView(savedInstanceState: Bundle?) {
        RoomServiceManager.init(requireActivity(),getRoomInfo())
...
//            }
    ...
    private fun requestActivities() {
        ...
        getAppConfigByKey<List<RoomActivityBean>>(api) {
            if (it.isEmpty()) {
                getActivityBannerView().visibility = View.INVISIBLE
            } else {
                    ...
                    adapter = object : BaseBannerAdapter<Any>() {
                        override fun bindData(
                            ...
                            holder?.findViewById<LinearLayout>(R.id.root)?.safeClick {
                                ...
                                if ((model.activeType ?: "002") == "001") {
                                    getUrlBySchema(model.schema)?.let { url ->
                                        BrowserDialog.newInstance("${url}?token=${MMKVProvider.loginResult?.token}&chatRoomId=${getRoomInfo().crId}&userId=${MMKVProvider.userId}")
                                            .show(childFragmentManager, "BrowserDialog")
                                    }
                                } else {
                                    handleSchema(model.schema, getRoomInfo().crId)
                                }
                            }
                        }
...
//                    refreshData(it.filter { item -> item.state == "001" && item.os == "001"&& (item.grade?.toInt() ?: -1 ).let { lever ->
//                        lever == -1 || lever<=MMKVProvider.consumeLevel
//                    }})
                    refreshData(it.filter { item ->
                        item.state == "001" && item.os == "001" && item.grade.let { s ->
                            val grade = if (s.isNullOrEmpty()) -1 else s.toInt()
                            grade == -1 || grade <= MMKVProvider.consumeLevel
                        }
                    })
                }.create()
            }
    ...
    private fun requestHistoryMessage() {
        ...
        RoomServiceManager.historyMessage.removeAll { t -> t.time < RoomServiceManager.quitRoomTime }
        ...
        messages.forEach {
            try {
                ...
                if (type == Constants.IMMessageType.MSG_TYPE_USER_IN) {
                    messageList.add(parseUserJoinMessage(parseCustomMessage(data.data)))
                }
                if (type == Constants.IMMessageType.MSG_TYPE_GIFT) {
                    val gift = parseCustomMessage<GiftMessage>(data.data)
                    (
                            if (gift.giftSource == "003") {
                                parseBoxGiftMessage(gift)
                            } else {
                                parseGiftMessage(gift, true)
                            }
                            ).let { messageModels ->
                ...
                if (type == Constants.IMMessageType.MSG_CHAT_WATER) {
                    if (MMKVProvider.room_water_info_show) {
                        messageList.add(parseWaterMessage(parseCustomMessage(data.data)))
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    private fun initChatRecyclerView() {
        ...
        val adapter = RoomChatRecyclerAdapter().apply {
            addType<RoomChatMessageModel> {
                when (type) {
                    ...
                    CHAT_MSG_TYPE_PD -> R.layout.room_chat_item_pd
            ...
            onBind {
                if(itemViewType == R.layout.room_chat_item_notice){
                    findView<TextView>(R.id.tv_content).apply {
                        movementMethod = LinkMovementMethod.getInstance()
                        highlightColor = Color.TRANSPARENT
                    }
                }else if(itemViewType == R.layout.room_chat_item_pd){
                    findView<TextView>(R.id.tv_content).safeClick {
                        getModel<RoomChatMessageModel>(modelPosition).let {
                            CommonConfirmDialog(context, title = "确定前往${it.roomName}吗",
                            ) {
                                if (this) {
                                    if (getRoomInfo().crId == it.roomId) {
                                        customToast("已在该房间")
                                    }else
        ...
        adapter.models = RoomServiceManager.allMessageList.filter { it.messageKindList.contains("001") }
    ...
    open fun gotoAitUser(chatMessageModel: RoomChatMessageModel){}
        ...
        mMicAction = {
            when (this) {
                ...
                MicActionDialog.ClickType.TYPE_CLEAN_INCOME -> {//清魅力值
                    ...
                    CharmValueCleanDialog.newInstance(getRoomInfo().crId,it.onMicroPhoneNumber){
                        if (this) {
    ...
    fun showRoomSettingDialog() {
        ...
        RoomSettingDialogV2.newInstance(getRoomInfo(), canBet).setCallBack {
            when (it) {
                ...
                RoomSettingClickType.TYPE_PD -> {
                    RoomPDSettingFragment.newInstance(getRoomInfo().crId,getRoomInfo().roomTitle).show(childFragmentManager)
                }

                RoomSettingClickType.TYPE_PK -> {
                    RoomPkSettingFragment.newInstance(getRoomInfo().crId).show(childFragmentManager)
                }
    ...
    open fun observerCustomMessage(attachment: BaseAttachment) {
        ...
        when (attachment.type) {
            ...
            Constants.IMMessageType.MSG_TYPE_ROOM_CHAT_CONTENT -> { //文本消息
                ...
                data.aitList?.let {
                    it.filter { userId ->
                        userId.userId == MMKVProvider.userId
                    }.let { list ->
                        if(list.isNotEmpty()){
                            tipUserAit()
                        }
                ...
                scope(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main){
                        if (data.giftSource == "003") {
                            ...
                            (parseGiftMessage(data,false).let {
                                if(it.isNotEmpty()){
                                    insertChatModel(it)
                                }
                            })
            ...
            Constants.IMMessageType.MSG_TYPE_KICK_MIC -> {//抱下麦消息
                val data = parseCustomMessage<UpMicIMMessage>(attachment.data)
                val model =
                    getRoomInfo().wheatPositionList.find { it.wheatPositionId == data.userId }
                if (model != null) {
                    clearMicInfo(model)
                }
                if (data.userId == MMKVProvider.userId) {
...
//            Constants.IMMessageType.MSG_CP ->{
//                val data = parseCustomMessage<CpMessage>(attachment.data)
//                val chatModel = RoomChatMessageModel(
//                    content = context?.let { data.getMessage(it) }, type = CHAT_MSG_TYPE_NOTICE,
//                    messageKindList =  mutableListOf("001", "002")
//                )
...
//            }//CP消息
            ...
            Constants.IMMessageType.MSG_TYPE_ROOM_NEW_BROADCAST_MESSAGE -> {
                ...
                updateRoomBroadMsg(data)
            }

            Constants.IMMessageType.MSG_TYPE_ROOM_NEW_BROADCAST_END -> {
                updateRoomBroadMsg(null)
            }

            Constants.IMMessageType.MSG_GAME_PRIZE -> {
                if(MMKVProvider.consumeLevel<2) return
                val data = parseCustomMessage<PrizeModel>(attachment.data)
                insertLocalModels(parsePrizeMessage(data))
            }

            //派单消息
            Constants.IMMessageType.MSG_ROOM_PD -> {
                val data = parseCustomMessage<PDModel>(attachment.data)
                insertChatModel(parsePDMessage(data))
            }

            //派单消息
            Constants.IMMessageType.MSG_ROOM_MQTT -> {
                connectMQTT(parseCustomMessage<RoomStartPkMessage>(attachment.data))
            }
        }
    }

    abstract fun connectMQTT(mqttConfig:RoomStartPkMessage)
...
//    }
    ...
    fun insertChatModel(model: RoomChatMessageModel, isHistory: Boolean = false) {
        if (!isHistory) {
            if (RoomServiceManager.allMessageList.size > 5000) {
                RoomServiceManager.allMessageList.removeAll(
                    RoomServiceManager.allMessageList.subList(
                        3,
                        300
                    )
                )
        ...
        if (model.messageKindList.contains(getChatTabIndex())) {
            getChatRecyclerView().models?.let {
                if (it.size > 5000) {
                    getChatRecyclerView().models = it.toMutableList().apply {
                        this.removeAll(it.subList(3, 300))
                        this.add(model)
                    }
                } else {
            ...
            //LogUtils.e("size ："+ (getChatRecyclerView().models?.size ?: 0))
    ...
    fun insertLocalModels(models: List<RoomChatMessageModel>, isHistory: Boolean = false) {

        for (model in models) {
            insertChatModel(model)
        }
//        if (!isHistory) {
//            RoomServiceManager.allMessageList.addAll(models)
//        }
//        getChatRecyclerView().addModels(mutableListOf(models))
//        getChatRecyclerView().scrollToBottom()
    }
    ...
    //向聊天列表插入送礼消息
    private fun parseGiftMessage(model: GiftMessage,isHistory: Boolean = false): MutableList<RoomChatMessageModel> {
        logE("parseGiftMessage")
        val messageList = mutableListOf<RoomChatMessageModel>()
        val sourceUser = model.sourceUser

        model.targetUsers.forEach {
            var messagesKings = model.messageKindList
            val ssb = buildSpannedString {
                ...
                it.giftInfos.forEach { gift ->
                    color(com.kissspace.module_common.R.color.color_FEC238.resToColor()) {
                        ...
                        if(gift.giftType == "106"||model.giftSource == "005"){
                        if (!isHistory) FlowBus.post(
                            Event.GiveGiftByCoinNet(
                                sourceUser,
                                it.nickName,
                                it.userId,
                                gift
                        ...
                        model.luckyBagGoldCoins?.let {
                            append(" 获得金币 ")
                            color(com.kissspace.module_common.R.color.color_FEC238.resToColor()) {
                                append("$it")
                            }
                            messagesKings = if (model.sourceUserChatRoomId != getRoomInfo().crId) {
                                if (it >= 10000) mutableListOf("001") else mutableListOf()
                            } else
                                if (it >= 10000) mutableListOf(
                                    "001",
                                    "002",
                                    "003"
                                ) else mutableListOf("001", "002")
                        } ?: run {
                            messagesKings = mutableListOf()
                        }
                    }
                }
            }
            if (messagesKings.isNotEmpty()) {
    ...
    //解析派单
    private fun parsePDMessage(messages: PDModel): RoomChatMessageModel {

        return RoomChatMessageModel(
            content = messages.content,
            roomId = messages.chatRoomId,
            roomName = messages.roomTitle,
            type = CHAT_MSG_TYPE_PD,
            messageKindList = messages.messageKindList
        )
    }

    fun parsePrizeMessage(messages: PrizeModel): ArrayList<RoomChatMessageModel> {
        val list = ArrayList<RoomChatMessageModel>()
        messages.giftInfo?.forEach {
            val ssb = buildSpannedString {
                append("在${it.gameName}中获得价值")
                color(com.kissspace.module_common.R.color.color_FEC238.resToColor()) {
                    append("${it.price.toInt()}金币")
                }
                append("的礼物")
                color(com.kissspace.module_common.R.color.color_D9853B.resToColor()) {
            ...
            list.add(
                RoomChatMessageModel(
                    profilePath = messages.profilePath,
                    userId = messages.userId,
                    nickname = messages.nickname,
                    headwearUrl = messages.headwearUrl,
                    wealthLevel = messages.consumeLevel,
                    charmLevel = messages.charmLevel,
                    type = CHAT_MSG_TYPE_NORMAL,
                    privilege = messages.privilege,
                    medalList = messages.medalList,
                    contentSpan = ssb,
                    messageKindList = mutableListOf("001", "003") //messages.messageKindList
                )
            )
        }
    ...
    override fun onEvent(t: List<ChatRoomMessage>?) {
        t?.forEach {
            try {
                val json = JSONObject(it.attachStr)
                logE("收到自定义消息${json}")
                logETag("收到自定义消息1",json.toString())
                val type = json.getString("type")
    ...
    abstract fun updateRoomBroadMsg(msg: RoomScreenMessageModel?)
     ...
     * 用户是否在麦上
     */
    fun isUserOnMic(userId: String?) =
        getRoomInfo().wheatPositionList.find { it.wheatPositionId == userId } != null

    fun findUserMic(userId: String?) =
        getRoomInfo().wheatPositionList.find { it.wheatPositionId == userId }
    ...
    open fun getRoomInfo():RoomInfoBean {
        return (requireParentFragment() as LiveAudioFragment).getRoomInfo()
    }
    ...
    override fun createDataObserver() {
        ...
        FlowBus.observerEvent<Event.CapturedSoundLevelUpdateEvent>(this) {
            if (it.soundLevel > 0) {
                val model = getRoomInfo().wheatPositionList.find { t -> t.wheatPositionId == MMKVProvider.userId }
                if(model != null) {
                    getMicrophoneLayout().playTalkAnimation(model)
                }
//                else {
//                    getMicrophoneLayout().stopTalking(model)
//                }
            }
        }
        ...
        FlowBus.observerEvent<Event.MsgPDEvent>(this) {
            insertChatModel(parsePDMessage(it.data))
        }
        ...
        //当电话进来时帮他下麦
        FlowBus.observerEvent<Event.PhoneInCome>(this) {
            findUserMic(MMKVProvider.userId)?.let {
                quitMic(it.onMicroPhoneNumber)
                CommonConfirmDialog(requireContext(), "电话接进,已自动帮您下麦"){}.show()
            }
//           if(isUserOnMic(MMKVProvider.userId)){
...
//           }
    ...
    open fun onGamePrizePlay(prizeModels: List<PrizeModel>?){
