...
class CommonJsBridge(private val block: (String, Any, Any?) -> Unit) {
    ...
    @JavascriptInterface
    fun gamesUserClick(members:String) {
        block(JSName.JSNAME_showGiftPopupAnimation,members,"")
    }
    ...
    @JavascriptInterface
    fun gamesGifts(list:String) {
        block(JSName.JSNAME_gamesGifts,list,null)
    }
}
...
object JSName {
    ...
    //显示礼物
    const val JSNAME_showGiftPopupAnimation = "showGiftPopupAnimation"
    ...
    //游戏礼物
    const val JSNAME_gamesGifts = "gamesGifts"
