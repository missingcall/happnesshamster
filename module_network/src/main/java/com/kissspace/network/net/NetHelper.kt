...
import com.kissspace.util.showKeyboard
...
fun netInit(
    ...
    val builder = OkHttpClient.Builder() //Utils.getUnsafeOkHttpClient() //
    builder.config()
...
fun NetCoroutineScope.netCatch(error: (AppException)-> Unit ){
    catch {
        scope {
            error.invoke(ExceptionHandle.parseException(it))
        }
    }
}
...
inline fun <reified T> ViewModel.requestPost(
    url: String,
    param: MutableMap<String, Any?>? = mutableMapOf(),
    state: MutableSharedFlow<ResultState<T>>
) {
    scopeNetLife {
        val result = doRequestPost<T>(url, param)
        state.emit(ResultState.onAppSuccess(result))
    }.catch { error ->
        scopeLife {
            state.emit(ResultState.onAppError(ExceptionHandle.parseException(error)))
...
suspend inline fun <reified T> doRequestPost(
    url: String,
    any: Any?,
): T {
    return coroutineScope {
        this.Post<T>(url) {
            json(GsonUtils.toJson(any))
        }.await()
    }
}
